...
\begin{exercise}[Subgrassmannians]
  Let $V$ be a vector space over a field $K$.
  Recall that the \emph{$k$-th Grassmannian} $G(k,V)$ of $V$ is the set of $k$-dimensional linear subspaces of $V$.
  \begin{enumerate}
    \item Let $U⊆V$ be a linear subspace.
      Find an inclusion $G(k,U)↪G(k,V)$ and describe its image.
    \item Let $W⊆V$ be a linear subspace of dimension $r≥0$.
      Find an inclusion $G(k-r,V/W) ↪ G(k,V)$ and describe its image.
    \item Let $H$ be a \emph{subquotient} of $V$, that is, a quotient of a linear subspace of $V$.
      Find an $r≥0$ together with an inclusion $G(k-r,H) ↪ G(k,V)$ and describe its image.
  \end{enumerate}
  Subsets of $G(k,V)$ of this form are called \emph{subgrassmannians}.

  Now, recall that we can regard $G(k,V)$ as a projective variety via the \emph{Plücker embedding} $ι:G(k,V) → ℙ(⋀^kV)$, sending a subspace spanned by $w_1,\dots,w_k$ to $[w_1∧\dots∧w_k$].
  \begin{enumerate}[start=4]
    \item Given linear subspaces $Γ,Λ⊆V$ with $\dim(Λ)=l$ and $s≥0$, prove that $\dim(Γ∩Λ)≥s$ if and only if for all $v_1,\dots,v_{l-s+1}∈Λ$ we have $ι(Γ)∧v_1∧\dots v_{l-s+1}=0$.
  \end{enumerate}
  Using this, conclude that the following subsets of $G(k,V)$ are closed subvarietes:
  \begin{enumerate}[start=5]
  ...
  \end{enumerate}
  Which of these subsets $Z⊆G(k,V)$ are \emph{linear sections} under the Plücker embedding, that is, for which of these is there a linear subspace $Θ⊆P(⋀^kV)$ such that $ι(Z)=ι(G(k,V))∩Θ$?
\end{exercise}
