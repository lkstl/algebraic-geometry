...
\begin{exercise}[More on irreducibility]
  Let $X$ be a topological space.
  \begin{enumerate}
    \item Show that the following statements are equivalent:
          \begin{enumerate}
            \item $X$ is irreducible.
            \item Any finite list of non-empty open subsets of $X$ has non-empty intersection.
            \item Every non-empty open subset of $X$ is dense in $X$.
            \item Every non-empty open subset of $X$ is connected.
            \item Every non-empty open subset of $X$ is irreducible.
          \end{enumerate}
          \item Show that a subspace $Y⊆X$ is irreducible if and only if its closure $\overline{Y}⊆X$ is irreducible.
  \end{enumerate}
\end{exercise}

\begin{exercise}[The projective closure]
  Embed affine space $𝔸^{n}_{K}$ in projective space $ℙ^{n}_{K}$ by identifying it with the open subset $U_{i} = \{[x_{0}:\ldots:x_{n}] \mid x_{i}≠ 0\}⊂ℙ^{n}_{K}$ (cf.~Exercise 19) for some fixed $i ∈ \{0,\ldots,n\}$.
  Given an affine variety $X⊆𝔸^{n}_{K}$, write $\overline{X}⊆ℙ^{n}_{K}$ for the closure of $X$ in $ℙ^{n}_{K}$ under this embedding and call it the \emph{projective closure} of $X$ in $ℙ^{n}_{K}$ (with respect to the $i$-th coordinate).
  \begin{enumerate}
    \item Observe that an affine variety is irreducible if and only if its projective closure is irreducible.
  \end{enumerate}
  Given $f ∈ K[T_{1},\dots,T_{n}]$ of degree $d$, we write $\overline{f} ∈ K[X_{0},\dots,X_{n}]_{d}$ for its homogenization $Ψ^{(d)}_{i}(f)$ with respect to $X_{i}$ (cf. Exercise 20).
  Moreover, if $𝔞⊆K[T_{1},\dots,T_{n}]$ is an ideal, we denote its homogenization by $\overline{𝔞}⊆K[X_{0},\dots,X_{n}]$, the ideal generated by all $\overline{f}$ for $f ∈ 𝔞$.
  \begin{enumerate}[start=2]
          \item Show that $\overline{𝒱(𝔞)}=𝒱_{+}(\overline{𝔞})$.
          \item Show that $X ↦ \overline{X}$ induces a Galois correspondence between the set of non-empty closed subvarieties $X$ of $𝔸^{n}_{K}$ and the set of closed subvarieties $Z$ of $ℙ^{n}_{K}$ with $Z∩U_{i}≠∅$.
  \end{enumerate}
\end{exercise}

\begin{exercise}[The twisted cubic]
  Let $X=\{(t,t^{2},t^{3}) \mid t ∈ K\}⊆𝔸^{3}_{K}$ be the \emph{twisted cubic} in affine $3$-space.
  \begin{enumerate}
    \item Show that $X$ is an irreducible closed subset of $𝔸^{3}_{K}$.
    \item Find generators of $ℐ(X)$ whose homogenization does not generate the vanishing ideal $ℐ(\overline{X})$ of the projective closure $\overline{X}$ (with respect to $X_{0}$).
  \end{enumerate}
\end{exercise}


% [GörtzWedhorn] Exercise 1.5
% [GörtzWedhorn] Exercise 1.24
