...
\begin{exercise}[Subgrassmannians]
  ...
  \begin{enumerate}
    ...
    Show the existence of an $r≥0$ such that for each $k≥0$ there is an injection $G(k,H) ↪ G(k+r,V)$.
      The image of such a map is called a \emph{subgrassmannian} or \emph{Grassmannian subvariety}.
    \item Show that the image of a subgrassmannian $G(k-r,H) ↪ G(k,V)$ under the Plücker embedding $ι:G(k,V) ↪ ℙ(⋀^k V)$ is a \emph{linear section} of $ι(G(k,V))$, that is, there is a linear subspace $Λ⊆ℙ(⋀^kV)$ such that $ι(G(k-r,H)) = ι(G(k,V)) ∩ Λ$.
      In particular, Grassmannian subvarieties are indeed projective varieties.
  \end{enumerate}
\end{exercise}
