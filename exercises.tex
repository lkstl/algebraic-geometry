...
\begin{exercise}[Preimages of projective varieties]
  Let $K$ be a field and $f:ℙ_K^m → ℙ_K^n$ be a regular map.
  Given a projective variety $Y⊆ℙ_K^n$, show that its preimage $f^{-1}(Y)⊆ℙ_K^m$ is a projective variety.
\end{exercise}

\begin{exercise}[Linear sections of Grassmannians]
  Let $V$ be a vector space over a field $K$.
  Recall that the \emph{$k$-th Grassmannian} $G(k,V)$ of $V$ is the set of $k$-dimensional linear subspaces of $V$.
  We regard it as a projective variety via the \emph{Plücker embedding} $ι:G(k,V) → ℙ(⋀^kV)$, sending a subspace spanned by $w_1,\dots,w_k$ to $[w_1∧\dots∧w_k$].
  \begin{enumerate}
    \item Let $H$ be a \emph{subquotient} of $V$, that is, $H$ is a quotient of a linear subspace of $V$.
      Show the existence of a natural number $r≥0$ together with an inclusion $G(k-r,H) ↪ G(k,V)$ and describe its image.
      A subset of this form is called a \emph{subgrassmannian} of $G(k,V)$.
    \item Let $Γ⊆V$ be a linear subspace with basis $γ_1,\dots,γ_k ∈ Γ$ and write $γ \coloneqq γ_1 ∧ \dots ∧ γ_k ∈ ⋀^kV$.
      Furthermore, let $Λ⊆V$ be linear subspace of dimension $\dim(Λ)=l$.
      Prove for all $s≥0$ that $\dim(Γ∩Λ)≥s$ if and only if for all $v_1,\dots,v_{l-s+1}∈Λ$ we have $γ∧v_1∧\dots∧v_{l-s+1} = 0$.
    \item Conclude that the following subsets of $G(k,V)$ are \emph{linear sections} under the Plücker embedding, that is, their image under $ι$ is the intersection of a linear subspace $Θ⊆ℙ(⋀^kV)$ with $ι(G(k,V))$.
      \begin{enumerate}
        \item The set $Σ_s(Λ)$ of those $k$-planes which meet a given linear subspace $Λ⊆V$ in a subspace of dimension at least $s≥0$.
        \item The set $Σ(Λ)$ of those $k$-planes which meet a given linear subspace $Λ⊆V$.
        \item A subgrassmannian $G(k-r,H)↪G(k,V)$.
      \end{enumerate}
  \end{enumerate}
\end{exercise}

\begin{exercise}[The nilpotent cone of $𝖌𝖑_2$]
  Identify the space $\Mat_2(K)$ of $2 × 2$ matrices over a field $K$ with $𝔸^4_K$ and denote its coordinate ring by $K[a,b,c,d]$.
  Show that the subset $\Nil_2(K)$ of nilpotent matrices is an irreducible closed affine cone in $𝔸^4$.
\end{exercise}
...
%\begin{exercise}[Geometry on $𝔾(1,3)$]
%  Let $K$ be a field and denote the standard basis of $K^4$ by $(e_1,e_2,e_3,e_4)$.
%  This induces the basis $(e_1∧e_2, e_1∧e_3, e_1∧e_4, e_2∧e_3, e_2∧e_4, e_3∧e_4)$ of $⋀^2K^4$, in particular we obtain an isomorphism $⋀^2K^4 ≅ K^6$ and hence an isomorphism on projectivizations $ℙ(⋀^2K^4) ≅ ℙ^5$.
%  Writing $G(2,4) \coloneqq G(2,K^4)$, the Plücker embedding of this Grassmannian can be regarded as a map $G(2,4) ↪ ℙ^5$.
%  Concretely it sends the span of two vectors
%  \begin{equation*}
%    v_1 = 
%    \begin{bmatrix}
%      v_{11} \\
...
%    \end{bmatrix}
%  \end{equation*}
%  to its \emph{Plücker coordinates} $[V_{12}:V_{13}:V_{14}:V_{23}:V_{24}:V_{34}]$, where $V$ is the $2 × 4$ matrix with $v_1$ and $v_2$ as rows, and $V_{ij}$ denotes the $(i,j)$-th 2-minor of this matrix.
%  \begin{enumerate}
%    \item Show that $ω ∈ ⋀^2K^4$ is decomposable (that is, of the form $ω=v∧w$) if and only if $ω∧ω=0$.
%      Conclude that the Plücker map embeds $G(2,4)$ as a quadric hypersurface into $ℙ^5$ by explicitly computing the defining equation in Plücker coordinates.
%  \end{enumerate}
%  From now on we write $𝔾\coloneqq𝔾(1,3)\coloneqq G(2,K^4)$.
%  \begin{enumerate}
%    \item For any point $p ∈ ℙ^3$ and any plane $H⊂ℙ^3$, let $Σ_{p,H}⊆𝔾$ be the subset of lines in $ℙ^3$ passing through $p$ and lying in $H$.
%      Show that $ι(Σ_{p,H}) ⊆ ℙ^5$ is a line, and that conversely each line in $ℙ^5$ is of this form for some $p∈ℙ^3$ and $H⊂ℙ^3$.
%    \item For any point $p∈ℙ^3$, let $Σ_p⊆𝔾$ be the subset of lines in $ℙ^3$ passing through $p$.
%      Similarly, for any plane $H⊂ℙ^3$ let $Σ_H⊆𝔾$ be subset of lines in $ℙ^3$ lying in $H$.
%      Show that both $ι(Σ_p)$ and $ι(Σ_H)$ are $2$-planes in $ℙ^5$, and that conversely any $2$-plane in $ℙ^5$ contained in $𝔾$ is either of the form $ι(Σ_p)$ for some $p$ or of the form $ι(Σ_H)$ for some $H$.
%    \item Let $ℓ_1,ℓ_2⊆ℙ^3$ be skew lines, i.e. non-intersecting non-parallel lines.
%      Show that the set $Q⊆𝔾$ of lines in $ℙ^3$ meeting both is the intersection of $𝔾$ with a $3$-plane in $ℙ^5$, and thus a quadric surface.
%      Deduce that $Q≅ℙ^1×ℙ^1$.
%  \end{enumerate}
%\end{exercise}
