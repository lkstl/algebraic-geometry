...
\begin{exercise}[The topology on projective space]
  ...
  \begin{enumerate}[start=2]
    ...
    \item Show that $(U_i)_{i ∈ \{0,\dots,n\}}$ is a decomposition of $ℙ^n_K$ satisfying the conditions of (a), thus defining a topology on $ℙ^n_K$.
    \item Prove that the map $π:𝔸^{n+1}_K∖\{0\} → ℙ^n_K$ sending a point to the line through $p$ and the origin is continuous with respect to the topology defined above.
...
\begin{exercise}[Affine cones]
  ...
  \begin{enumerate}
    \item Denote the set of homogeneous ideals of $R$ which are not equal to the irrelevant ideal by $\Idl_+(R)$ and write $π:𝔸^{n+1}_K∖\{0\} → ℙ^n_K$ for the canonical projection.
      ...
      \end{tikzcd}\]
      Here, $C(T)$ is the affine cone $π^{-1}(T)∪\{0\}$ associated to a subset $T⊆ℙ^n_K$ of projective space and $\widetilde{∃}_π$ sends a subset $X ⊆ 𝔸^{n+1}_K$ to $π(X∖\{0\})$.
