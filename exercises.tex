...
\begin{exercise}[Monomial ideals and their dimension]
  Let $K$ be a field and $𝖆⊆K[X_1,\dots,X_n]$ an ideal.
  Recall that $𝖆$ is a \emph{monomial ideal} if it admits a generating set consisting of monomials.
  \begin{enumerate}
    \item Show that $𝖆$ is monomial if and only if for each finite family of monomials $f_1,\dots,f_n$ with $f_1 + \dots + f_n ∈ 𝖆$, we have $f_i ∈ 𝖆$ for all $i$.
    \item Show that the radical of a monomial ideal is monomial.
    \item A monomial ideal is \emph{square-free} if it admits a generating set consisting of \emph{square-free monomials}, i.e. monomials which are not divisible by the square of any variable.
      Show that a monomial ideal is square-free if and only if it is radical.
    \item Given a monomial $f=X_{i_1}^{e_1}\cdots X_{i_m}^{e_m}$ with $e_j > 0$, define $\sqrt{f} \coloneqq X_{i_1}\cdots X_{i_m}$.
      Show that $$\sqrt{(f_1,\dots,f_r)} = (\sqrt{f_1},\dots,\sqrt{f_r}).$$
    \item Conclude that the vanishing set of a monomial ideal is a union of linear subspaces.
