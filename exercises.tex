...
\begin{exercise}[Spectra see more than classical varieties]
  ...
  Do the following tasks for $K=ℂ,ℝ,ℚ$:
  \begin{enumerate}
    \item Factor $f$ into irreducible polynomials.
    \item List the points of $𝒱(f)⊆𝔸^1_K$ on the classical affine line.
    \item List the points of $𝒱(f)⊆\Spec(K[X])$ in the spectrum of $K[X]$.
  \end{enumerate}
\end{exercise}
