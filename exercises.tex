...
\begin{exercise}[Suprema, Infima und Idealoperationen]
  ...
  ∀a' ∈ A: (∀m ∈ M:m ≤ a') ⇒ a ≤ a'
  $$
  Dually $a$ is a \emph{meet} or \emph{infimum} of $M$ if it is a join of $M$ in the poset $(A,≥)$ with reversed ordering.
  \begin{enumerate}
    \item Show that meets and joins are unique if they exist.
      In case of existence, we write $⋁ M$ for the join of $M$ and $⋀ M$ for its meet.
    \item Let $◯:A → A$ be a closure operator on a poset $(A,≤)$.
      \begin{enumerate}
        \item Show that $◯(a) = ⋀ \{x ∈ \Fix(◯) \mid a ≤ x\}$ for all $a ∈ A$.
      \end{enumerate}
      Regard $\Fix(◯)$ as a subposet of $(A,≤)$ via the inclusion $i:\Fix(◯)↪A$.
      To distinguish between meets in $\Fix(◯)$ and meets in $A$ we write $⋀^{\Fix(◯)}$ respectively $⋀^A$ (and similarly for joins).
      Let $M⊆\Fix(◯)⊆A$ and suppose that meet $⋀^A M$ and join $⋁^A M$ exist in $A$.
      \begin{enumerate}
        \item $⋀^{\Fix(◯)}M = ⋀^A M$, in particular $⋀^A M$ is a fixed point.
        \item $⋁^{\Fix(◯)} M = ◯ (⋁^A M)$.
      \end{enumerate}
