...
\begin{exercise}[The topology on projective space]
  \begin{enumerate}
    \item Let $X=⋃_{i ∈ I}X_i$ be a decomposition of a set $X$ into subsets, each endowed with a topology $τ_i$, and write $X_{ij}\coloneqq X_i ∩ X_j$ and%
      ...
      Show that there exists a unique topology $τ$ on $X$ with $τ|_i \coloneqq τ|_{X_i} = τ_i$ if and only if the following two conditions are satisfied for all $i,j ∈ I$.
      \begin{enumerate}
        \item $X_{ij}$ is open in $X_i$ and in $X_j$.
        \item $τ_i|_{ij} = τ_j|_{ij}$.
      \end{enumerate}
  \end{enumerate}
  Recall that the topology on $ℙ^n_K$ was defined by decomposing $ℙ^n_K$ into 
\end{exercise}
