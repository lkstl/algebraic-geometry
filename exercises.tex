...
\begin{exercise}[The variety of nilpotent matrices]
  ...
  Denote its coordinate ring by $R \coloneqq K[X_{i,j}]_{i,j ∈ \{1,\dots,n\}}$ and write $\Nil_n(K)$ for the set of nilpotent $n × n$ matrices.
  Prove:
  \begin{enumerate}
    \item The set $\Nil_n(K)$ is a variety.
    \item The vanishing ideal $ℐ(\Nil_n(K))$ is generated by the set $\{s_1,\dots,s_n\}$, where $s_{n-r}\coloneqq\tr(⋀^{n-r} G)$ is the sum of all principal $r$-minors of the \emph{generic $n × n$ matrix}
      $$
      G \coloneqq
      \begin{bmatrix}
      ...
      ∈ \Mat_n(R).
      $$
    \item The variety $\Nil_n(K)$ is irreducible.
