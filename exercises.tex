...
\begin{exercise}[Finite difference calculus]
  Recall that the \emph{backward difference operator} $Δ$ on the abelian group of functions $h:ℤ → ℤ$ is defined by $Δh(x)\coloneqq h(x) - h(x-1)$.
  \begin{enumerate}
    \item Let $h:ℤ → ℤ$ and suppose there is a $d≥0$ such that $Δ^d h$ is constant.
      Show that there exists a polynomial $H ∈ ℚ[X]$ such that for all $x ∈ ℤ$ we have $H(x)=h(x)$.
    \item If $I⊆K[x_1,\dots,x_n]$ is a monomial ideal with Hilbert function $h_I$, prove the existence of a polynomial $\mathrm{HP}_I ∈ ℚ[X]$ and a bound $e$ such that $h_I(q)=\mathrm{HP}(q)$ for all $q≥e$.
  \end{enumerate}
\end{exercise}

\begin{exercise}
  Consider the monomial ideal $I=(x)∩(y)∩(x,y)^{r+1}$ for $r ∈ ℕ$.
  Show:
  \begin{enumerate}
    \item $I=(x^ry,x^{r-1}y^2,\dots,x^2y^{r-1},xy^r)$.
    \item The Hilbert series $\mathrm{HS}_I$ of $I$ is given by $(1-z)^2\mathrm{HS}_I(z) = 1-rz^{r+1}+(r-1)z^{r+2}$.
    \item The Hilbert polynomial $\mathrm{HP}_I$ is the constant polynomial $2$.
  \end{enumerate}
  What is the dimension of $I$ and what is its degree?
\end{exercise}
